...
def detect_secrets_scan(repo_path):
    ...
    command = f"detect-secrets scan --exclude-files '({repo_path}/.git/*|.*/target/.*|.*/build/.*|.*/out/.*|.*/bin/.*|.*/node_modules/.*|.*/bower_components/.*)'"
...
def find_sensitive_strings(repo_path):
    """Search for sensitive strings in the files of the given repository directory."""
        ...
        for filename in files:
            if filename.endswith('.java'):
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    content = file.read()
                    # Add your logic here to search for sensitive strings within the content
                    # For example, you can use regex or custom checks.
