...
# RSA private key (replace with your actual private key securely)
RSA_PRIVATE_KEY = """
...
# RSA public key (for verification if needed)
RSA_PUBLIC_KEY = """
-----BEGIN PUBLIC KEY-----
xxx
-----END PUBLIC KEY-----
...
def reforge_token():
    ...
    try:
        # Decode the token without verification to extract the payload
        decoded_token = jwt.decode(original_token, RSA_PUBLIC_KEY, algorithms=["RS256"], options={"verify_signature": False})
        print("Decoded token payload:")
        print(decoded_token)
    except jwt.InvalidTokenError as e:
        print(f"Error: Invalid token. {e}")
    ...
    # Re-sign the token using the RSA private key
    reforged_token = jwt.encode(decoded_token, RSA_PRIVATE_KEY, algorithm="RS256")
