...
# Optional dependency for webhook notifications
try:
    import requests
except ImportError:
    requests = None
...
class SecurityValidator:
    """Validates request content and origin against security policies."""
    
    # Pre-compiled patterns for common XSS/injection vectors
    SUSPICIOUS = [
    ...
    def validate_ip(self, ip_str: str) -> bool:
        """Check if IP is valid and not in a restricted range (for High security)."""
        try:
    ...
    def validate_content(self, content: str) -> tuple[bool, list[str]]:
        """Scan decoded content for known malicious patterns."""
...
class RateLimiter:
    """Thread-safe rate limiting implementation based on client IP."""
    ...
    def get_stats(self) -> dict[str, int]:
        with self.lock:
            return {
                ...
                "recent_reqs": sum(len(ts) for ts in self.requests.values())
            }

class Decoder:
    """Handles recursive decoding of URL and Base64 encoded strings."""
    ...
    def decode(self, data: str) -> tuple[str, list[tuple[LayerType, str]]]:
        ...
        for _ in range(self.max_depth):
            # Prevent potential DoS via memory expansion
...
class FileManager:
    """Manages secure file operations for captured data."""
    
    def __init__(self, save_dir: Path):
        self.save_dir = save_dir
        # Create directory with restricted permissions (rwxr-x---)
        self.save_dir.mkdir(exist_ok=True, mode=0o750)
        self.lock = threading.Lock()

    def save(self, content: str, ts: str, ext: str) -> Path | None:
        """Saves content to disk with a sanitized filename."""
        fname = re.sub(r'[<>:"/\\|?*]', '_', ts)[:100]
        fpath = self.save_dir / f"{'decoded' if ext == 'html' else 'raw'}_{fname}.{ext}"
        try:
            # Path traversal prevention: Ensure file remains within save_dir
            ...
            with self.lock:
                ...
                # Set file permissions to read/write for owner only (rw-r-----)
                fpath.chmod(0o640)
            return fpath
        except Exception as e:
            logging.error(f"File save operation failed: {e}")
            return None
...
class CTFReceiverHandler(http.server.SimpleHTTPRequestHandler):
    """
    Custom request handler for processing exfiltrated data.
    Configuration is injected as class variables to ensure compatibility with socketserver.
    """
    cfg: ClassVar[Config]
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        """Override default logging to use standard logging module."""
        logging.info(f"{self.client_address[0]} - {fmt % args}")

    def _auth_check(self) -> bool:
        """Validate IP allowlist, API keys, and rate limits."""
    ...
    def _process_payload(self, raw: str):
        """Decode, validate, log, and save payload data."""
        ...
        if threats:
            logging.warning(f"Security threats detected from {ip}: {threats}")
            if not is_safe:
                self.stats.add_request(ip, blocked=True)
                return
        ...
        if self.cfg.webhook_url and is_html and requests:
            ...
            except Exception as e: logging.error(f"Webhook notification failed: {e}")
...
def main():
    ...
    p.add_argument('--port', '-p', type=int, default=80, help='Port to listen on')
    p.add_argument('--show', '-s', action='store_true', dest='show_html', help='Display HTML content in console')
    p.add_argument('--show-all', action='store_true', help='Display all decoded content')
    p.add_argument('--show-full-preview', action='store_true', help='Do not truncate log previews')
    p.add_argument('--json-log', action='store_true', help='Enable JSON logging')
    p.add_argument('--allow-ip', nargs='*', default=None, help='List of allowed IP addresses')
    p.add_argument('--security-level', choices=[l.value for l in SecurityLevel], default='medium', help='Security validation level')
    p.add_argument('--api-key', help='Require X-API-Key header')
    p.add_argument('--webhook-url', help='URL for webhook notifications')
    p.add_argument('--ssl-cert', help='Path to SSL certificate')
    p.add_argument('--ssl-key', help='Path to SSL private key')
    p.add_argument('--save-dir', default='decoded_html', help='Directory for saved files')
    p.add_argument('--rate-limit', type=int, default=100, help='Max requests per window')
    p.add_argument('--rate-window', type=int, default=60, help='Rate limit window in seconds')
    p.add_argument('--no-stats', action='store_true', help='Disable statistics endpoint')
    p.add_argument('--log-level', default='INFO', help='Logging verbosity')
    args = p.parse_args()
    ...
    # Inject configuration and dependencies into Handler class
    ...
    # Allow address reuse to prevent binding errors on restart
    ...
    print(f"Server listening on port {cfg.port}")
    print(f"Security: {cfg.security_level.value.upper()} | Save Directory: {cfg.save_dir}")
