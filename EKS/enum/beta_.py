...
def get_ec2_instance_info(cluster_name, regions):
...
def main():
    # Replace 'your-cluster-name' with the actual EKS cluster name
    cluster_name = input("Enter the EKS cluster name: ")

    # Ask for regions (comma-separated)
    regions = input("Enter the AWS regions (comma-separated): ").split(',')

    # Remove leading/trailing whitespaces from regions
    regions = [region.strip() for region in regions]

    get_ec2_instance_info(cluster_name, regions)
