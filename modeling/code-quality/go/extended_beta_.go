...
func isSafeMatch(rule, path, line string) bool {
	...
	case "Template Injection":
		return strings.Contains(line, "|") && (strings.Contains(line, "|escape") ||
...
func getCodeContext(filePath string, lineNum int, contextSize int) string {
	...
	// Join context and then truncate to a maximum of 300 characters if needed.
	contextStr := strings.Join(context, "\n")
	maxContext := 300
	if len(contextStr) > maxContext {
		contextStr = contextStr[:maxContext] + "\n… [context truncated]"
	}
	return contextStr
}
...
// Improved scanFile function with better context handling and truncation of match and context
	...
	for scanner.Scan() {
		...
		for _, r := range rules {
			...
			if r.Pattern.MatchString(line) {
				// Check for false positives using post-match safe check
				...
				// Extract the actual match and truncate if it's too long
				match := r.Pattern.FindString(line)
				maxLen := 100
				if len(match) > maxLen {
					match = match[:maxLen] + "… [truncated]"
				}
