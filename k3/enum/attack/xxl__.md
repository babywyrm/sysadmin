...
### Enhanced kubernetes-api-pentest.sh

```bash
#!/usr/bin/env bash
#
# kubernetes-api-pentest.sh - Comprehensive K8s API enumeration
# Usage: bash kubernetes-api-pentest.sh [--deep] [--stealth] [--output DIR]
#
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --stealth) STEALTH_MODE=true; DELAY=5; shift ;;
...
if [[ -f /var/run/secrets/kubernetes.io/serviceaccount/token ]]; then
  ...
  if [[ -z "${TOKEN:-}" ]] || [[ -z "${APISERVER:-}" ]]; then
    echo "[!] Not in cluster and TOKEN/APISERVER not set"
    exit 1
  fi
  echo "[+] Using external credentials"
fi

# Helper functions
log() {
  echo "[$(date +'%H:%M:%S')] $*" | tee -a "$SCAN_DIR/scan.log"
}
...
api_get() {
  ...
  [[ $STEALTH_MODE == true ]] && sleep $((RANDOM % DELAY + 1))
  
  curl -sk --max-time 30 \
    -H "Authorization: Bearer $TOKEN" \
    -H "Accept: application/json" \
    "$APISERVER$endpoint" > "$output" 2>/dev/null || {
    echo "{\"error\": \"Request failed\"}" > "$output"
  }
}
...
check_permission() {
  ...
  curl -sk --max-time 10 \
    ...
    -H "Content-Type: application/json" \
    -X POST "$APISERVER/apis/authorization.k8s.io/v1/selfsubjectaccessreviews" \
    -d "$body" 2>/dev/null | jq -r '.status.allowed' 2>/dev/null || echo "false"
}

# Banner
cat <<'EOF' | tee "$SCAN_DIR/banner.txt"
╔═══════════════════════════════════════════╗
║   Kubernetes API Pentest Tool v2.0       ║
║   Mi Familia Edition                      ║
╚═══════════════════════════════════════════╝
EOF
...
api_get "/version" "$SCAN_DIR/cluster-version.json"

log "Getting API resources..."
api_get "/api/v1" "$SCAN_DIR/core-api.json"
api_get "/apis" "$SCAN_DIR/api-groups.json"

log "Enumerating namespaces..."
api_get "/api/v1/namespaces" "$SCAN_DIR/namespaces.json"
NAMESPACES=$(jq -r '.items[]?.metadata.name' "$SCAN_DIR/namespaces.json" 2>/dev/null || echo "default")
...
log "Testing permissions..."
{
  ...
  for resource in pods secrets services deployments; do
    for verb in get list create delete; do
      allowed=$(check_permission "$verb" "$resource")
      echo "$resource,$verb,$allowed"
    done
  done
} > "$SCAN_DIR/permissions.csv"

log "Testing dangerous permissions..."
{
  echo "Permission,Allowed,Risk"
  while IFS=, read -r perm risk; do
    IFS=: read -r verb resource <<< "$perm"
    allowed=$(check_permission "$verb" "$resource")
    echo "$perm,$allowed,$risk"
  done <<'PERMS'
create:pods/exec,CRITICAL - Remote Code Execution
get:secrets,HIGH - Credential Access
list:secrets,HIGH - Credential Discovery
create:pods,MEDIUM - Pod Creation
create:persistentvolumeclaims,MEDIUM - Storage Access
get:nodes,LOW - Infrastructure Discovery
PERMS
} > "$SCAN_DIR/dangerous-permissions.csv"

# Phase 3: RBAC Enumeration
log "=== Phase 3: RBAC Enumeration ==="

log "Enumerating service accounts..."
api_get "/api/v1/serviceaccounts" "$SCAN_DIR/serviceaccounts.json"

log "Enumerating roles..."
