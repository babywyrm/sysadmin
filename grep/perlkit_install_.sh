#!/usr/bin/env bash
# install-ptk.sh - The All-in-One Perl Toolkit Installer ..kinda..
# Version: 1.1.0
#
# This script contains the PTK source code embedded within it.
# It will check dependencies, extract the code, and install it to your path.
...
if [[ -t 1 ]]; then
    ...
    GREEN='' BLUE='' RED='' YELLOW='' BOLD='' RESET=''
fi

# --- HEADER ---
echo -e "${BLUE}${BOLD}"
echo "╔═══════════════════════════════════════════════════════════╗"
echo "║             Perl Toolkit (ptk) Installer v1.1             ║"
echo "╚═══════════════════════════════════════════════════════════╝"
echo -e "${RESET}"

# --- STEP 1: CHECK PLATFORM & PERL ---
echo -e "${BOLD}➜ Checking System Requirements...${RESET}"

OS="$(uname -s)"
if [[ "$OS" == "Darwin" ]]; then
    echo -e "  • Detected: ${GREEN}macOS${RESET}"
elif [[ "$OS" == "Linux" ]]; then
    echo -e "  • Detected: ${GREEN}Linux${RESET}"
else
    echo -e "  • Detected: ${YELLOW}$OS${RESET} (Warning: Untested platform)"
fi

if ! command -v perl &> /dev/null; then
    echo -e "${RED}✗ Perl is not installed.${RESET}"
    exit 1
fi

PERL_VER=$(perl -e 'print $^V' | sed 's/v//')
echo -e "  • Perl Version: ${GREEN}v${PERL_VER}${RESET}"

# Simple version check logic
if [[ "$(printf '%s\n' "$REQUIRED_PERL" "$PERL_VER" | sort -V | head -n1)" != "$REQUIRED_PERL" ]]; then
    echo -e "${YELLOW}⚠ Warning: Perl v${REQUIRED_PERL} is recommended. You have v${PERL_VER}.${RESET}"
    echo "  Some modern features might not work, but we will proceed."
fi

# --- STEP 2: CHECK MODULES ---
echo -e "\n${BOLD}➜ Checking Perl Modules...${RESET}"
MISSING_MODULES=()
for mod in List::Util Time::Piece JSON::PP Getopt::Long Pod::Usage; do
    if perl -M$mod -e 1 &> /dev/null; then
        echo -e "  • $mod: ${GREEN}Found${RESET}"
    else
        echo -e "  • $mod: ${RED}Missing${RESET}"
        MISSING_MODULES+=("$mod")
    fi
done

if [ ${#MISSING_MODULES[@]} -ne 0 ]; then
    echo -e "\n${YELLOW}⚠ Missing modules detected.${RESET}"
    echo "Attempting to install them via cpanm (if available) or standard cpan..."
    
    if command -v cpanm &> /dev/null; then
        cpanm "${MISSING_MODULES[@]}"
    else
        echo "cpanm not found, using cpan (this might take a minute)..."
        cpan "${MISSING_MODULES[@]}"
    fi
fi

# --- STEP 3: EXTRACT AND INSTALL ---
echo -e "\n${BOLD}➜ Installing ptk...${RESET}"
...
# Extract the Perl script from below the __PAYLOAD__ marker
echo "  • Extracting source code to $TARGET_FILE..."
awk '/^__PAYLOAD__$/{flag=1;next} flag' "$0" > "$TARGET_FILE"

if [ -s "$TARGET_FILE" ]; then
    chmod +x "$TARGET_FILE"
    echo -e "  • Permissions set to ${GREEN}executable${RESET}"
else
    echo -e "${RED}✗ Extraction failed. File is empty.${RESET}"
    exit 1
fi
...
if [[ ":$PATH:" == *":$INSTALL_DIR:"* ]]; then
    echo -e "  • ${GREEN}Success!${RESET} $INSTALL_DIR is in your PATH."
else
    echo -e "  • ${YELLOW}Action Required:${RESET} $INSTALL_DIR is NOT in your PATH."
    echo -e "    Run this command to fix it for this session:"
    echo -e "      ${BOLD}export PATH=\"$INSTALL_DIR:\$PATH\"${RESET}"
    echo -e "    To make it permanent, add that line to your ~/.bashrc or ~/.zshrc"
fi

# --- FOOTER ---
echo -e "\n${GREEN}${BOLD}✔ Installation Complete!${RESET}"
echo -e "Run '${BOLD}ptk help${RESET}' to get started."
exit 0

# =========================================================
# THE PERL SOURCE CODE STARTS BELOW THIS MARKER
# DO NOT EDIT BELOW THIS LINE
# =========================================================
__PAYLOAD__
#!/usr/bin/env perl
# ptk - Perl Toolkit 2025
# A comprehensive CLI utility collection for text processing
