#!/usr/bin/env python3
"""
mcp-audit v4 — Hardened Universal MCP + DVMCP Security Scanner
...
MCP_INIT_PARAMS = {
    ...
    "capabilities": {},
    "clientInfo": {"name": "mcp-audit", "version": "4.0"},
}
...
DANGEROUS_TOOL_PATTERNS = {
    "shell_exec":    (r"(shell|exec|run|execute|cmd|bash|sh|powershell|eval|system)", "CRITICAL"),
    "filesystem":    (r"(read_file|write_file|delete|remove|mkdir|listdir|readdir|glob|file_read|file_write)", "HIGH"),
    "network":       (r"(fetch|curl|wget|http_get|http_post|request|socket|connect|http_request)", "HIGH"),
    "database":      (r"(sql|query|database|db_exec|mongo|redis|execute_query|db_query)", "HIGH"),
    "code_eval":     (r"(eval|exec|compile|__import__|subprocess|popen|code_exec)", "CRITICAL"),
    "secrets_access":(r"(secret|credential|password|token|key|vault|ssm|aws_secret)", "HIGH"),
    "cloud_api":     (r"(iam|s3|ec2|gcp|azure|k8s|kubectl|terraform|cloud_exec)", "HIGH"),
    "process_mgmt":  (r"(kill|signal|fork|spawn|process|proc_exec)", "MEDIUM"),
}
...
RAC_PATTERNS = {
    "reverse_shell":  (r"(nc|ncat|socat|netcat|bash\s+-i|/dev/tcp|reverse.?shell)", "CRITICAL"),
    "port_forward":   (r"(port.?forward|tunnel|socks|proxy\s+port)", "HIGH"),
    "remote_desktop": (r"(vnc|rdp|teamviewer|anydesk|screenshare)", "HIGH"),
    "c2_beacon":      (r"(beacon|c2|command.and.control|meterpreter|cobalt.?strike|sliver|havoc)", "CRITICAL"),
...
class TargetResult:
    ...
    def add(self, check: str, severity: str, title: str,
            detail: str = "", evidence: str = "") -> Finding:
...
class MCPSession:
    ...
    Fixes the v4 bug where _probe_sse_endpoint() opened a throwaway
    connection, then MCPSession opened a second one with a different
    session_id causing all POSTs to be routed to the wrong session.

    Now the persistent SSE listener captures the endpoint event itself.
    """

    def __init__(self, base: str, sse_path: str, timeout: float = 25.0):
        self.base = base
        self.sse_url = base + sse_path
        self.post_url: str = ""          # populated by listener
    ...
    def _listen(self):
        ...
        try:
                ...
                for raw in resp.iter_lines():
                    ...
                    if line.startswith("event:"):
                        ...
                        if event_type == "endpoint" and data:
                            # Capture the real session-bound POST URL
                            self.post_url = (
            ...
            self._connected.set()       # unblock waiters on error
            self._endpoint_ready.set()

    def wait_ready(self, timeout: float = 10.0) -> bool:
        """Block until endpoint event received or timeout."""
        return self._endpoint_ready.wait(timeout=timeout)

    def call(self, method: str, params: dict | None = None,
             timeout: float | None = None, retries: int = 2) -> dict | None:
        wait = timeout or self.timeout
        for attempt in range(retries + 1):
            self._req_id += 1
            payload = _jrpc(method, params, self._req_id)
            dbg(f"→ POST [{attempt}] {method} id={self._req_id} → {self.post_url}")
            try:
                r = self._client.post(
                    self.post_url, json=payload,
            ...
            # Wait for matching SSE response
            ...
            for m in pending:
                self._q.put(m)
            dbg(f"  timeout waiting for id={self._req_id} (attempt {attempt})")
            if attempt < retries:
                time.sleep(1.0)
        return None
...
# ════════════════════════════════════════════════════════════════
# Transport detection  (bug-fixed)
# ════════════════════════════════════════════════════════════════

def detect_transport(url: str, connect_timeout: float = 25.0) -> MCPSession | None:
    parsed = urlparse(url)
    base = f"{parsed.scheme}://{parsed.netloc}"
    hint = parsed.path.rstrip("/") or None

    paths = ([hint] if hint else []) + [p for p in SSE_PATHS if p != hint]
    vrb(f"Trying SSE paths: {[p for p in paths if p is not None]}")

    # ── SSE probe: open ONE persistent connection per path ────────────────
    for sse_path in paths:
        if sse_path is None:
            continue

        # Quick availability check (non-streaming)
        try:
            probe_client = httpx.Client(verify=False, timeout=5)
            r = probe_client.get(
                base + sse_path,
                headers={"Accept": "text/event-stream"},
                timeout=4,
            ...
            probe_client.close()
            if r.status_code != 200:
                dbg(f"  {sse_path} → HTTP {r.status_code}, skipping")
                continue
            if "text/event-stream" not in r.headers.get("content-type", ""):
                dbg(f"  {sse_path} → not event-stream, skipping")
                continue
        except Exception as e:
            dbg(f"  {sse_path} probe error: {e}")
            continue

        # Open the real persistent session — listener captures endpoint event
        dbg(f"  Opening persistent MCPSession on {sse_path}")
        session = MCPSession(base, sse_path, timeout=connect_timeout)

        if session.wait_ready(timeout=10.0) and session.post_url:
            vrb(f"SSE negotiated: {sse_path} → {session.post_url}")
            return session

        dbg(f"  No endpoint event on {sse_path}, closing")
        session.close()

    # ── HTTP POST fallback ────────────────────────────────────────────────
    vrb("SSE failed — trying plain HTTP POST fallback")
    ...
    for path in POST_PATHS:
        ...
        try:
            ...
            dbg(f"  HTTP probe {path} → {r.status_code}")
...
def enumerate_server(session: MCPSession, result: TargetResult):
    ...
    if not resp or "result" not in resp:
        result.add("init", "HIGH", "No response to MCP initialize",
    ...
    vrb(f"Server: {info.get('name','?')} v{info.get('version','?')} caps={list(caps.keys())}")

    result.add("auth", "HIGH", "Unauthenticated MCP initialize accepted",
    ...
    with _time_check("prompt_injection", result):
        ...
        for tool in result.tools:
            ...
            for prop, pdef in tool.get("inputSchema", {}).get("properties", {}).items():
                _scan(pdef.get("description", ""), f"tool '{name}' param '{prop}'")
...
def check_tool_poisoning(result: TargetResult):
    with _time_check("tool_poisoning", result):
        for tool in result.tools:
            name = tool.get("name", "")
            full = tool.get("description", "") + " " + json.dumps(tool.get("inputSchema", {}))
            ...
            for pat in POISON_PATTERNS:
                if re.search(pat, full, re.IGNORECASE | re.DOTALL):
                    result.add("tool_poisoning", "CRITICAL",
                               ...
                               f"Pattern: {pat}", evidence=full[:400])
                    break
...
def check_tool_shadowing(all_results: list[TargetResult], result: TargetResult):
                    ...
                    for pat in INJECTION_PATTERNS + POISON_PATTERNS:
                        if re.search(pat, text, re.IGNORECASE | re.DOTALL):
                            result.add("indirect_injection", "CRITICAL",
                                       ...
                                       f"Pattern: {pat}", evidence=text[:400])
                            break
            ...
            for pat in TOKEN_THEFT_PATTERNS:
                if re.search(pat, combined, re.IGNORECASE):
                    result.add("token_theft", "CRITICAL",
                               ...
                               f"Pattern: {pat}", evidence=combined[:300])
                    break
    ...
    with _time_check("attack_chains", result):
        ...
        for a, b in ATTACK_CHAIN_PATTERNS:
            if a in checks and b in checks:
                result.add("attack_chain", "CRITICAL",
                           f"Attack chain: {a} → {b}",
...
def check_sse_security(base: str, sse_path: str, result: TargetResult):
    with _time_check("sse_security", result):
        ...
        try:
            r = client.get(base + sse_path,
                           headers={"Accept": "text/event-stream",
                                    "Origin": "https://evil.example.com"}, timeout=5)
            ...
            if isinstance(v, str):
                if "PRIVATE KEY" in v:
                    GLOBAL_K8S_FINDINGS.append(Finding(
                        target="k8s", check="helm_secrets", severity="CRITICAL",
                        title=f"Private key in Helm values: {sname} → {np}"))
                elif any(s in k.lower() for s in ["password","secret","token","apikey"]):
                    GLOBAL_K8S_FINDINGS.append(Finding(
                        target="k8s", check="helm_secrets", severity="HIGH",
    ...
    if secrets_data:
        for secret in secrets_data.get("items", []):
            ...
            try:
                ...
                _scan_helm(sname, json.loads(decoded).get("chart", {}).get("values", {}), "")
            except Exception:
                pass
...
def scan_target(url: str, all_results: list[TargetResult],
                args: argparse.Namespace) -> TargetResult:
    ...
    transport_label = "SSE" if isinstance(session, MCPSession) and session.sse_url else "HTTP"
    result.transport = transport_label
    console.print(f"  [green]✓[/green] Transport={transport_label}  post_url={session.post_url}")
    ...
    console.print(f"  [dim]Tools={len(result.tools)} "
    ...
    session.close()
    result.timings["total"] = time.time() - t_start
    console.print(f"  [dim]Done in {result.timings['total']:.1f}s  "
...
def run_parallel(urls: list[str], args: argparse.Namespace) -> list[TargetResult]:
    ...
    task = progress.add_task(f"Scanning {len(urls)} target(s)", total=len(urls))
...
def print_report(results: list[TargetResult]):
    console.print("\n")
    console.rule("[bold]MCP AUDIT v4 — REPORT[/bold]")

    all_findings = [f for r in results for f in r.findings] + GLOBAL_K8S_FINDINGS
    ...
    sorted_f  = sorted(all_findings, key=lambda f: sev_order.get(f.severity, 5))
    ...
    for r in ranked:
        score = r.risk_score()
        color = ("bold red" if score >= 20 else
                 "red"      if score >= 10 else
                 "yellow"   if score >= 5  else "green")
        pt.add_row(
            r.url.replace("http://", ""),
            r.transport,
            str(len(r.tools)),
    ...
    if VERBOSE and results:
        ...
        for r in ranked[:3]:
            ...
            for check, elapsed in sorted(r.timings.items(),
    ...
    if chain_findings:
        ...
        for f in chain_findings:
            console.print(f"  [bold red]⚠[/bold red]  {f.title} ({f.target})")
...
def expand_targets(args: argparse.Namespace) -> list[str]:
    ...
    if args.port_range:
        ...
        if m:
            ...
            console.print("[red]Invalid --port-range format. Use: host:start-end[/red]")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="mcp-audit v4: Universal MCP + DVMCP Security Scanner",
...
Examples:
  ...
  python3 mcp_audit.py --port-range localhost:9001-9010 --verbose
  python3 mcp_audit.py --port-range localhost:9001-9010 --debug --json report.json
  python3 mcp_audit.py --targets http://localhost:2266 --mode full
        """,
    )
    parser.add_argument("--targets",    nargs="+", metavar="URL")
    parser.add_argument("--port-range", metavar="HOST:START-END")
    parser.add_argument("--mode",       choices=["mcp","internal","full"], default="mcp")
    parser.add_argument("--namespace",  default="default")
    parser.add_argument("--workers",    type=int, default=5)
    parser.add_argument("--timeout",    type=int, default=25)
    parser.add_argument("--json",       metavar="FILE")
    parser.add_argument("--verbose", "-v", action="store_true",
                        help="Show per-check progress and details")
    parser.add_argument("--debug",      action="store_true",
                        help="Show raw JSON-RPC messages and SSE diagnostics (implies --verbose)")
    ...
    console.print(Panel.fit(
        "[bold cyan]mcp-audit v4[/bold cyan] — Hardened Universal MCP + DVMCP Security Scanner\n"
        f"  mode={args.mode}  workers={args.workers}  timeout={args.timeout}s  "
    ...
    if not targets and args.mode not in ("internal", "full"):
        parser.print_help()
        sys.exit(1)
    ...
    if targets:
        console.print(
            ...
            + "  ".join(t.replace("http://", "") for t in targets[:8])
            + ("  …" if len(targets) > 8 else "")
        )
        results = run_parallel(targets, args)
